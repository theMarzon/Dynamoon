...
for (const _event of eventsLoaded) {
    ...
    intentSources = intentSources.concat(_event.intents);

    // Importa los intentos de los archivos que contiene el evento
    for (const _all of eventsSource[_event.name].all) {

        intentSources = intentSources.concat(_all.intents);
    };
};

// Elimina los intentos repetidos
intentSources = intentSources.filter((val, ind, arr) => arr.indexOf(val) === ind);

// Exporta los intentos
module.exports = intentSources;